...
- Wallet 使用韩系 ins 风展示用户收支、角色独立经济账户与最近心意，并可进入 Shop；Shop 支持角色店铺、分类搜索、共同愿望单、共享购物车、商城晒单及订单记录。线上角色可以像发送转账一样直接发送已付款的外卖、礼物或购物订单卡片，不要求用户设置地址、确认付款或使用用户钱包；订单会写入角色账本并随时间推进配送状态。
- Shop 保留可离线使用的默认商品，并提供角色“AI 上新”：文本模型仅根据用户与角色资料生成店铺设定、四件商品、现实价格、晒单文案和英文商品摄影提示词，不读取或注入普通聊天上下文。每张商品卡可按需调用 VOOM 当前选择的 OpenAI、NovelAI 或 Pollinations 生图模型，生成结果会保存到本地商城数据并进入完整备份；页面不会在打开时自动批量生图，避免无提示消耗 API 额度。
- Shop 与线上单聊双向联动：用户可把商品、愿望单商品、角色店铺和商城晒单发给选中的角色；角色通过 API 共同挑选后会把选择理由与商品卡以角色身份发进其单聊；AI 上新会发送店铺卡，用户结算共享购物车后会发送订单卡。聊天卡保存当时的商品快照并可回跳具体商品、店铺、晒单或订单，后续线上回复能够理解双方实际发送过的卡片；角色钱包金额不会作为隐藏上下文注入普通聊天。
- Fanfic 以当前用户和所选角色的真名作为双主角：可从内置原创题材、联网趋势标签或自定义题材开始，设置视角、基调、结局、篇幅与内容边界，再生成封面、书名、简介、全书大纲和每章约 2500 字的连载小说。人物原设定只用于一次性抽象创作 DNA，后续写作不读取原设定、聊天、记忆或世界书；职业、背景、地点、关系起点、事件与世界规则均从零原创。
- Fanfic 每章正文、高潮锚点与该章虚构读者评论在同一次模型请求中生成，并在全部成功后一起写入 IndexedDB；第一章同时生成整本小说评论。阅读器支持高潮段评论弹层、回复、点赞、字号/纸张设置、阅读位置恢复、前后章导航与自选下一章方向，封面在未配置图片模型时使用本地程序封面。
